...
class SlimUNETRBlock_v2(nn.Module):
    def __init__(self, dim, d_state = 16, d_conv = 4, expand = 2, head=4, num_slices=4, step = 1):
        ...
        if self.step != 3 and self.step != 2:
            self.compression = nn.Conv3d(in_channels=dim, 
                    out_channels=dim, 
                    kernel_size=3, 
                    stride=4, 
                    padding=0)
            self.upsample = nn.ConvTranspose3d(in_channels=dim, 
                                        out_channels=dim, 
                                        kernel_size=3, 
                                        stride=4, 
                                        padding=0, 
...
class SlimMambaEncoder(nn.Module):
    def __init__(self, in_chans=4, kernel_sizes=[4, 2, 2, 2], depths=[1, 1, 1, 1], dims=[48, 96, 192, 384], num_slices_list = [64, 32, 16, 8],
        ...
        for i in range(4):
            ...
            stage = nn.Sequential(
                *[SlimUNETRBlock_v2(dim=dims[i], num_slices=num_slices_list[i], head = heads[i], step=i) for j in range(depths[i])]
            )
...
class SlimUNETR(nn.Module):
    def __init__(self, in_chans=4, out_chans=3, kernel_sizes=[4, 2, 2, 2], depths=[1, 1, 1, 1], dims=[48, 96, 192, 384], heads=[1, 2, 4, 4], hidden_size=768, num_slices_list = [64, 32, 16, 8],
                out_indices=[0, 1, 2, 3]):
