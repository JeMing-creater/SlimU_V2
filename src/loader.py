...
if __name__ == '__main__':
    ...
    config = EasyDict(yaml.load(open('/workspace/SlimUNETR/config.yml', 'r', encoding="utf-8"), Loader=yaml.FullLoader))
    ...
    for i, batch in enumerate(train_loader):
        ...
        if batch['label'][0][0].max() == 0:
            nan_num += 1
            print('this label T is nan')
        if batch['label'][0][1].max() == 0:
            nan_num += 1
            print('this label V is nan')
    print(nan_num)
