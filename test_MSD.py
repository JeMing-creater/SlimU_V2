...
from save_MSD_singel_result import get_MSD_singel_result
best_acc = 0
best_class = []
...
if __name__ == '__main__':
    ...
    # 加载预训练模型
    model = load_pretrain_model(f"{os.getcwd()}/model_store/{config.finetune.checkpoint}/best/new/pytorch_model.bin", model,
                                accelerator)

    model, optimizer, scheduler, train_loader, val_loader = accelerator.prepare(model, optimizer, scheduler,
                                                                                train_loader, val_loader)
    # model.load_state_dict(torch.load(f"{os.getcwd()}/model_store/{config.finetune.checkpoint}/best/new/model.pth"))
    # # 开始验证
    ...
    get_MSD_singel_result(config=config,model=model,image_choose='hepaticvessel_025',device=accelerator.device)
    ...
    # torch.save(model.state_dict(), f"{os.getcwd()}/model_store/{config.finetune.checkpoint}/best/new/val_model.pth")
